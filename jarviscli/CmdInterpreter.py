...
class JarvisAPI(object):
    ...
    Plugins will receive an instance of this as the second (non-self) parameter
    of the exec()-method.
    ...
    def try_do(self, s):
        try:
            ...
            if self._api.is_spinner_running():
                self.spinner_stop("It seems some error has occur")
            print(
                Fore.MAGENTA
...
class CmdInterpreter(Cmd):
    ...
    def close(self):
        ...
        if self._api.is_spinner_running():
            self._api.spinner_stop('Some error has occur')
